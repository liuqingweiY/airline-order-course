...
DROP TABLE IF EXISTS `orders_lqw`;
DROP TABLE IF EXISTS `app_users_lqw`;

CREATE TABLE `app_users_lqw` (
...
CREATE TABLE `orders_lqw` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_orders_user_id` FOREIGN KEY (`user_id`) REFERENCES `app_users_lqw` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `app_users_lqw` (`id`, `username`, `password`, `role`) VALUES
(1, 'admin', '$2a$10$xlRrgsrwZL.ZPV.Dw92ch.JTlIBftkMSK0kRECUw7YpSs51haPQy.', 'ADMIN'),
(2, 'user', '$2a$10$xlRrgsrwZL.ZPV.Dw92ch.JTlIBftkMSK0kRECUw7YpSs51haPQy.', 'USER');

-- 插入覆盖所有场景的订单数据
INSERT INTO `orders_lqw` (`order_number`, `status`, `amount`, `creation_date`, `user_id`) VALUES
...
SELECT COUNT(*) AS '用户总数' FROM `app_users_lqw`;
SELECT status, COUNT(*) AS '订单数量' FROM `orders_lqw` GROUP BY status;
